...
/**
 * Integration test for package:provider:list command
 */
class ProviderListTest extends BaseIntegrationTest
{
    ...
    public function testProviderListExecutesSuccessfully()
    {
    ...
    public function testProviderListReturnsValidJson()
    {
        $providerName = 'IntegrationTestProvider_' . uniqid();
        $providerId = $this->createProvider($providerName);
        ...
        foreach ($data['results'] as $row) {
            if (isset($row['name']) && $row['name'] === $providerName) {
                $found = true;
                break;
            }
        }

        $this->assertTrue($found, 'Created provider not found in list results.');
