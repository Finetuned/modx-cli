...
/**
 * Integration test for plugin:disabled command
 */
class PluginDisabledTest extends BaseIntegrationTest
{
    ...
    public function testPluginDisabledListExecutesSuccessfully()
    {
        $pluginName = 'IntegrationTestPlugin_' . uniqid();
        $this->createPlugin($pluginName, 1);
    ...
    public function testPluginDisabledListReturnsValidJson()
    {
        $pluginName = 'IntegrationTestPlugin_' . uniqid();
        $this->createPlugin($pluginName, 1);
        ...
        $this->assertTrue($found, 'Disabled plugin not found in list results.');
    }
